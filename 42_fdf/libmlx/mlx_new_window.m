  ...
  if (event_funct[2] != NULL)
    {
      ...
      if (!(keyflag & NSctrlKeyMask) && (flag&NSctrlKeyMask)) event_funct[2](0xFF+3, event_param[2]);
  ...
  if (event_funct[3] != NULL)
    {
      ...
      if ((keyflag & NSctrlKeyMask) && !(flag&NSctrlKeyMask)) event_funct[3](0xFF+3, event_param[3]);
  ...
  if ((self = [super initWithFrame:rect pixelFormat:pixFmt]) != nil)
    {
      ...
      // Window ctrller subclass to set title
      // NSWindowctrller* windowctrller = [[NSWindowctrller alloc] initWithWindow:win];
      // [windowctrller windowTitleForDocumentDisplayName:title];
      // [windowctrller showWindow:nil];
      // MlxWinctrller *mlxWinCont = [[MlxWinctrller alloc] initWin:win andTitle:title];

      // after nswindowctrller who will retake first responder
